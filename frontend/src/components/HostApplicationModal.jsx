...
const HostApplicationModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const applicationStatus = existingApplication?.status || "none";

  const modalTitle = !isEditMode
    ? "Apply to Become a Host"
    : applicationStatus === "pending"
    ? "Edit Host Application"
    : applicationStatus === "needs_more_info"
    ? "Update Host Application"
    : applicationStatus === "rejected"
    ? "Re-Apply to Become a Host"
    : "Edit Host Application";

  const modalDescription = !isEditMode
    ? "Submit your owner verification details to start listing your property."
    : applicationStatus === "pending"
    ? "Your application is under review. You can update your details if needed."
    : applicationStatus === "needs_more_info"
    ? "Your application needs some corrections. Update the required details and submit again."
    : applicationStatus === "rejected"
    ? "Your previous application was rejected. You can correct the details and apply again."
  ...
  const getSuccessMessage = () => {
    if (!isEditMode) {
      return "Host application submitted successfully.";
    }

    if (applicationStatus === "pending") {
      return "Host application updated successfully.";
    }

    if (applicationStatus === "needs_more_info") {
      return "Host application updated and resubmitted successfully.";
    }

    if (applicationStatus === "rejected") {
      return "Host application re-applied successfully.";
    }
  ...
  const validateForm = () => {
    ...
    if (formData.pan_card_image && !formData.business_name.trim()) {
      newErrors.business_name =
        "Business name is required if PAN card image is uploaded.";
    }

    if (!isEditMode) {
      if (!formData.citizenship_front_image) {
        newErrors.citizenship_front_image =
  ...
  const normalizeBackendErrors = (data) => {
    ...
    Object.keys(data).forEach((key) => {
      ...
      if (Array.isArray(value)) {
        ...
        if (firstNestedKey) {
          const nestedValue = value[firstNestedKey];
          formatted[key] = Array.isArray(nestedValue)
            ? nestedValue[0]
            : nestedValue;
        }
      }
  ...
  return (
    ...
    >
      <div className="modal-content" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="modal-header">
          <h2 id="host-application-title">{modalTitle}</h2>
        ...
        {loadingExisting ? (
          ...
          <form onSubmit={handleSubmit} className="host-form">
            ...
            {isEditMode && existingApplication?.status === "needs_more_info" ? (
              <div className="info-box">
                <p>
                  Your previous application needs more information. Update the
                  required details and submit again.
                </p>
                ...
                <div className="verification-checks-grid">
                  <div
                    className={`check-item ${
                      existingApplication.phone_verified_check ? "passed" : "pending"
                    }`}
                  >
                    <i
                      className={`bi ${
                        existingApplication.phone_verified_check
                  ...
                  <div
                    className={`check-item ${
                      existingApplication.identity_verified_check
                        ? "passed"
                        : "pending"
                    }`}
                  >
                  ...
                  <div
                    className={`check-item ${
                      existingApplication.property_verified_check
                        ? "passed"
                        : "pending"
                    }`}
                  >
                  ...
                  <div
                    className={`check-item ${
                      existingApplication.bank_verified_check ? "passed" : "pending"
                    }`}
                  >
                    <i
                      className={`bi ${
                        existingApplication.bank_verified_check
                          ? "bi-check-circle-fill"
                          : "bi-info-circle"
                      }`}
                    ></i>
            ...
            {isEditMode && existingApplication?.status === "rejected" ? (
              <div className="info-box">
                <p>
                  Your previous application was rejected. Please correct the
            ...
            {isEditMode && existingApplication?.status === "pending" ? (
              <div className="info-box">
                <p>
                  Your application is currently under review. You can still update
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                <label htmlFor="citizenship_front_image">
