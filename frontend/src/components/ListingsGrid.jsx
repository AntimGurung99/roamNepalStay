...
const ListingCard = ({ listing, onClick }) => {
    ...
    const images = listing.all_images && listing.all_images.length > 0 
        ? listing.all_images 
    ...
    return (
        <div className="listing-card" onClick={() => onClick(listing.id)}>
            <div className="listing-image-container">
                <WishlistHeart listingId={listing.id} initialIsWishlisted={listing.is_wishlisted} />
                {images.length > 0 ? (
                    <>
                        <img 
                            src={images[currentImageIndex]?.startsWith('http') ? images[currentImageIndex] : `http://127.0.0.1:8000${images[currentImageIndex]}`} 
                            alt={listing.title} 
                        />
                        {images.length > 1 && (
                            <>
                                <button className="image-arrow left" onClick={handlePrev}>&lt;</button>
                                <button className="image-arrow right" onClick={handleNext}>&gt;</button>
                            </>
                        )}
                    </>
                ) : (
            ...
            <div className="listing-card-info">
                ...
                <div className="listing-card-location">
                    <i className="bi bi-geo-alt"></i> {listing.city}, {listing.region}, {listing.province}
                </div>
    ...
    const fetchListings = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                // Extremely robust parsing for any API format
    ...
    const handleCardClick = async (listingId) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                console.error('Failed to fetch listing details on Home Page:', response.status, errorMsg);
            }
        } catch (error) {
            ...
            <div className="listings-section">
                <div className="listings-grid">
                    {listings.length > 0 ? (
                        ...
                        <div className="no-listings">No published listings found. Admin needs to publish some!</div>
                    )}
                </div>
            </div>
