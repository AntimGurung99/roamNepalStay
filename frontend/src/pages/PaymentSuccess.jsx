  ...
  useEffect(() => {
    ...
    const verifyPayment = async () => {
      try {
        ...
        const provider = isEsewaRoute ? "esewa" : searchParams.get("provider");
        ...
        if (provider === "khalti") {
          ...
          setMessage(
            response?.data?.detail || "Khalti payment verified successfully."
          );
        } else if (provider === "esewa") {
          const esewaBookingId = bookingId || searchParams.get("booking_id");

          if (!esewaBookingId) {
            throw new Error("Missing eSewa booking ID.");
          }

          const response = await paymentsAPI.verifyEsewaPayment({
            booking_id: esewaBookingId,
          });

          setMessage(
            response?.data?.detail || "eSewa payment verified successfully."
          );
        } else {
          throw new Error("Unknown payment provider.");
        }
